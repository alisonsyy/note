...
- **添加元素之前需要调用 ensureCapacityInternal() 方法来判断容量是否足够**，传入一个参数也就是我们需要的容量(当前容量+1)
  ...
  - 之后根据minCapacity (我们需要的容量)调用ensureExplicitCapacity(minCapacity) 方法判断是否需要扩容。
    - 如果当前elementData数组的长度大于minCapacity，说明容量充足，不要扩容，就不需要继续下去了。
    - **如果当前elementData数组的长度小于minCapacity，需要调用grow()方法进行扩容。**
      ...
      - **调用`Arrays.copyOf()`将把原数组整个复制到新数组中。**
- **将我们要添加的元素添加到数组中。**
...
## 1.4.1 comparable和comparator的区别
...
## 1.4.2 比较HashSet,LinkedHashSet和TreeSet三者的异同
...
1. **数据结构：**首先将数据分为一段一段的存储，然后给每一段数据配一把锁，当一个线程占用锁访问其中一个段数据时，其他段的数据也能被其他线程访问。
